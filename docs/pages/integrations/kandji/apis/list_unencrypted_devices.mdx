...
Get a list of all unencrypted devices in an organization from Kandji. A device is classified as unencrypted when it has at least one unencrypted volume.
...
```python
from admyral.actions import list_kandji_unencrypted_devices
```
...
A JSON array of unencrypted devices.
...
```python
devices = list_kandji_unencrypted_devices(
