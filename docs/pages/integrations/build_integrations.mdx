...
2. Define the integration object in the `INTEGRATIONS` object. You need to define the following fields:
    ...
    - `credentials`: The necessary fields which are needed to authenticate with the integration
...
4. In the newly created file `<integration_name>.rs`, we need to implement the `IntegrationExecutor` trait. Therefore, we create a new struct with the name `<IntegrationName>Executor` (e.g., `JiraExecutor`). The struct should implement the `IntegrationExecutor` trait. The `IntegrationExecutor` trait has one function `execute` which takes the following parameters:
    ...
    const INTEGRATION_NAME: &str = "Jira"; // TODO: Adjust the integration name here

    #[derive(Debug, Clone, Serialize, Deserialize, PartialEq, Eq)]
    pub struct JiraExecutor; // TODO: Rename this struct according to the integratoin. Naming Convention: <IntegrationName>Executor
...
6. Additionally, we adapt the `execute` function of the `IntegrationExecutor` trait to fetch the secret by adding the line `let credential = fetch_secret::<T>(context, credential_name).await?;` where `T` is the name of the struct we just implemented (e.g., `JiraCredential`):
...
7.  Again, we need to add a key-value pair with the file name as key and display name of the API as value to the `_meta.json` file in the `apis` directory, so that the API is displayed in the navigation bar:
