...
You can open the Admyral UI by visiting `http://localhost:3000` (if no custom port was specified for `ADMYRAL_WEB_PORT`) in your browser.
...
<Tabs items={['CLI', 'Admyral UI']}>
    ...
    <Tabs.Tab>
        Go to Admyral running on [localhost:3000](localhost:3000) (if no custom port was specified for `ADMYRAL_WEB_PORT`). On the bottom left, click on settings and add your VirusTotal API key:
...
<Tabs items={['Code', 'No-Code']}>
    <Tabs.Tab>
        ...
        You can now go visit Admyral running on `http://localhost:3000` (if no custom port was specified for `ADMYRAL_WEB_PORT`). There, you should find the workflow and can inspect the workflow inside the No-Code editor.

    </Tabs.Tab>

    <Tabs.Tab>

        1. Visit `http://localhost:3000` (if no custom port was specified for `ADMYRAL_WEB_PORT`) in your favorite browser.
