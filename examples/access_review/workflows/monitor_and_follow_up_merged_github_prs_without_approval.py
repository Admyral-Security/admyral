...
def handle_merged_github_pr_without_approval(payload: dict[str, JsonValue]):
    ...
    if line_changes_count < 50:
        ...
        is_major_change = openai_chat_completion(
            ...
            "Major changes include adding new features or functions, changes to functionality (logic, functions, APIs, etc.), new APIs, or potentially breaking changes.\n"
            "Minor changes on the other hand would be adding / fixing a comment, documentation, renaming a parameter, hence changes that don't influence the functionality of the code.\n"
            "Your task is to classify the following git diff together with its summary whether the changes include major or minor changes. \n"
            "If the changes are major, please answer with 1 and nothing else. Otherwise, if the changes are minor, please answer with 2 and nothing else. \n"
            "Here is the git diff and its summary:\n"
            f"<diff>\n{commit_diff_cleaned}\n</diff>\n\n"
            f"<Summary>\n{git_diff_summary}\n</Summary>\n",
            stop_tokens=["\n"],
            secrets={"OPENAI_SECRET": "openai_secret"},
